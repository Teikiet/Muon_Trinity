...
# Treat the 200 time frames as input CHANNELS to a 2D CNN: (N, T, 16, 16)
X_t = torch.from_numpy(X)                  # (N, T, H, W)
...
class CareRegressor(nn.Module):
    def __init__(self, in_ch=T, n_out=5, p=0.3):
        super().__init__()
...
}, CACHE_PATH.replace(".npz", "_model.pt"))
print("Saved model.")