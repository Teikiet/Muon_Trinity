...
def extream_zenith(theta, Nh, mag = 0, h_obs_m=2944, h_max_m=112800.0, E_min=1e1, E_max=1e7):
    mceq.set_theta_deg(90)
    ...
    h = np.sin(np.radian(180-theta))*(6371e3 + h_obs_m)

    min_X = mceq.density_model.h2X(h_max_m*1e2)
    max_X = mceq.density_model.h2X(0)
    X_obs = mceq.density_model.h2X(h_obs_m*1e2)
    X_grid_1 = np.linspace(X_obs, max_X, Nh)
    X_grid_2 = np.linspace(X_obs, max_X, Nh)
...
def get_spectrum(theta, Nh, mag=0, h_obs_m=2944, h_max_m=112800.0, E_min=1e1, E_max=1e7):
    ...
    X_grid = np.linspace(min_X, max_X, Nh) #np.linspace(min_X, max_X,Nh)##
    #X_grid = np.ones(Nh)*np.mean([min_X, max_X])
    H_grid = mceq.density_model.X2h(X_grid)
    L_grid = np.array([earth_geom.delta_l(h, angle_rad) for h in H_grid])
    mceq.solve(int_grid=X_grid)
    longitudinal_spectrum = [
        mceq.get_solution('total_mu-', grid_idx=idx, mag=mag) + mceq.get_solution('total_mu+', grid_idx=idx, mag=mag) for idx in range(len(X_grid))]
    ...
    return d_CDF, L_grid, X_grid, H_grid, sum_mceq, CDF


# --- Grid settings ---
Nh    = 1000#1000
Nzen  = 9#1000
min_zen, max_zen = 0, 90
min_E,  max_E    = 1, 1e10
...
for theta in zen_centers:
    ...
    d_CDF, L_grid, X_grid, H_grid, sum_mceq, CDF = get_spectrum(
...
# --- Pickle it ---
out_path = f'/uufs/chpc.utah.edu/common/home/u1520754/Muon_Trinity/data/mceq_grid_{min_zen}_{max_zen}deg_Nzen{Nzen}.pkl' #_no_decay_no_E_loss
