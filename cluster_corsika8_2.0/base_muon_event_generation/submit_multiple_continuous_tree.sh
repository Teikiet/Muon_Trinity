...
SLURM_SCRIPT="$HOME/Muon_Trinity/cluster_corsika8_2.0/base_muon_event_generation/run_corsika8_trinity_chain_tree.slurm"
SIM_INPUT_SCRIPT="$HOME/Muon_Trinity/cluster_corsika8_2.0/Trinity_sim_input.py"
...
if [ -n "${ONLY_ENERGY}" ]; then
    ...
    if [ "${FOUND_ENERGY}" = false ]; then
        echo "ERROR: --only-energy '${ONLY_ENERGY}' not found in Trinity_sim_input.py energy list."
        exit 1
    fi
fi
if [ -n "${ONLY_SEED}" ]; then
    ...
    if [ "${FOUND_SEED}" = false ]; then
        echo "ERROR: --only-seed '${ONLY_SEED}' not found in Trinity_sim_input.py seed list."
        exit 1
    fi
fi
