#!/usr/bin/env python3
"""
build_trigger_resample_chunk.py

Worker script: reads a JSON chunk file containing triggered events,
processes resample positions for each, writes a result CSV.
"""
...
def make_path(base_path, pid, energy_str, zen, az, h, tel_x, tel_y, tel_z, radius, seed):
    ...
    pos_dir = f"x{fmt(tel_x)}_y{fmt(tel_y)}_z{fmt(tel_z)}"
    return os.path.join(base_path, energy_group, tree_root,
                        f"zen{fmt(zen)}", f"az{fmt(az)}", f"h{fmt(h)}",
                        pos_dir, "CARE")
...
def read_metrics(care_dir):
    ...
    try:
        ...
        pulse_width = float((above_half[-1] - above_half[0]) * SAMPLES_PER_NS) if len(above_half) >= 2 else 0.0

        peak_val = summed[peak_step]
        t10 = np.where(summed[:peak_step+1] >= 0.1 * peak_val)[0]
        t90 = np.where(summed[:peak_step+1] >= 0.9 * peak_val)[0]
        rise_time = float((t90[0] - t10[0]) * SAMPLES_PER_NS) if (len(t10) > 0 and len(t90) > 0) else 0.0

        time_spread = float(np.std(pixel_peak_time[hit_mask])) if n_hit >= 2 else 0.0
...
def main():
    ...
    with open(args.chunk_file, "r") as f:
        triggered_events = json.load(f)

    all_tel_positions = [(tx, tz) for tx in args.tel_x_values for tz in args.tel_z_values]
    ...
    with open(args.output, "w", newline="") as csvfile:
        ...
        for event in triggered_events:
            ...
            # Write base event row
            writer.writerow(event)
            rows_written += 1

            # Resample positions (skip base 0,0)
    ...
    print(f"Chunk done: {rows_written} rows -> {args.output}")
